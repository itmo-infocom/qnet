...
//Реализация исправления ошибок по алгоритму Хэминга.
//На вход подаётся битовый массив ключа. Функция возвращает последовательность контрольных бит, которые впоследствии необходимо передать декодировщику
vector<bool> HammingCode(vector<bool> key);

//Принимает на вход ключ key и последовательность контрольных бит cbit_storage_input
//Возвращает ключ с исправленной ошибкой.
//Алгоритм способен устранить не более одной ошибки (один бит) двойные и тройные ошибки он не распознаёт.
